...
const TechnicianDetails = () => {
    ...
    const { id } = useParams();
    const { data: technician, loading, error } = useFetchData(`${BASE_URL}/technicians/${id}`);
    ...
    const {
        ...
        ticketPrice = '',
        photo = ''
    } = technician || {};  // Ensure technician is not null
    ...
    return (
        <section>
            <div className="max-w-[1170px] px-5 mx-auto">
                {loading && <Loader />}
                {error && <Error />}
                {!loading && !error && technician && (  // Ensure technician is not null before rendering
                    <div className="grid md:grid-cols-3 gap-[50px]">
                        <div className="md:col-span-2">
                            ...
                            <div className="mt-[50px]">
                                {tab === 'about' && <TechnicianAbout name={name} about={about} qualifications={qualifications} experiences={experiences} />}
                                {tab === 'feedback' && <Feedback reviews={reviews} totalRating={totalRating}/>}
                            </div>
                        </div>
                        <div>
                            <SidePanel
                                technicianId={technician?._id}  // Access technician._id safely
