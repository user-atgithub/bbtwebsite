    ...
    <tr key={item._id}>
      <td className="px-6 py-4 text-gray-900" style={{ minWidth: '235px', wordBreak: 'break-word' }}>
        <div className="flex items-center">
          <img
            src={item.user?.photo || 'https://via.placeholder.com/40'} // Use optional chaining
            className="w-10 h-10 rounded-full"
            alt={item.user?.name || 'No photo'} // Use optional chaining
          />
          <div className="pl-3">
            <div className="text-base font-semibold" style={{ maxWidth: '150px', overflowWrap: 'break-word' }}>
              {item.user?.name || 'Unknown User'} // Fallback for name
            </div>
            <div className="text-normal text-gray-500" style={{ maxWidth: '150px', overflowWrap: 'break-word' }}>
              {item.user?.email || 'No email'} // Fallback for email
            </div>
          </div>
        </div>
      </td>
